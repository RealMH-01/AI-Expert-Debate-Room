...
输出格式要求：
整个回复必须是一个可被 JSON.parse 解析的 JSON 对象。
不要 Markdown 代码块；不要解释；不要在 JSON 前后输出任何字符。
Markdown 只允许出现在 message 字符串字段内部；如果需要分点，请写在 message 字符串内。
所有字符串必须正确转义，JSON 字段之间必须有逗号。
字段如下：
{
...
7. 攻击性：${agent.aggression ?? 50}/100（0=温和 100=极度攻击性）`
  ...
  if (input.structuredOutputRetry) {
    return [
      { role: 'system', content: system },
      {
        role: 'user',
        content: `上一轮输出不是合法 JSON。请只返回一个 JSON 对象，不要解释，不要 Markdown 代码块，不要在 JSON 前后输出任何字符。
必须符合以下 schema，并确保 JSON 字段之间有逗号、所有字符串正确转义。

议题：${userQuestion}

上一轮解析错误：${input.structuredOutputRetry.previousError || 'JSON.parse failed'}

schema:
{
  "message": "专家本轮完整发言正文；如需 Markdown，只能写在这个字符串内部",
  "claims": [
    { "claim_text": "本轮自己提出的核心观点，最多 3 条" }
  ],
  "attacks": []
...
6. 如需分点或小标题，只能写在最终 JSON 的 message 字符串字段内部`
