/**
 * OpenAI-Compatible Adapter (Generic Fallback)
 *
 * For user-specified OpenAI-compatible endpoints (openai_compatible provider).
 * Uses standard OpenAI Chat Completions format with no provider-specific logic.
 *
...
export class OpenAICompatibleAdapter extends BaseAdapter {
  ...
  constructor(model: string, thinkingEnabled: boolean = false) {
    super('openai_compatible', model, thinkingEnabled)
  ...
  protected async callApi(request: ProviderRequest): Promise<ProviderResponse> {
    return requestQueue.enqueue(this.providerId, async () => {
      ...
      if (!config) throw new Error(`Provider "openai_compatible" 未配置。请在设置中配置 API Key 和 Base URL。`)
      if (!config.apiKey) throw new Error(`Provider "openai_compatible" 缺少 API Key。`)
      if (!config.enabled) throw new Error(`Provider "openai_compatible" 已禁用。`)
      if (!config.baseUrl) throw new Error(`Provider "openai_compatible" 缺少 Base URL。请在设置中配置。`)
      ...
      try {
        console.log(`[OpenAICompatibleAdapter] 调用 ${this.model} - endpoint: ${baseUrl}`)
        ...
        return {
          ...
          reasoningText: data.choices[0].message.reasoning_content ?? undefined,
          usage: data.usage
            ? {
        ...
        if (error instanceof Error) {
          if (error.name === 'AbortError') {
            throw new Error(`Provider "openai_compatible" 请求超时 (${timeout / 1000}s)。`)
          }
          throw new Error(this.sanitizeError(error.message))
        }
        throw new Error('Provider "openai_compatible" 调用失败: 未知错误')
      } finally {
  ...
  async testConnection(): Promise<TestConnectionResult> {
    const config = getProviderConfig(this.providerId)
    if (!config) return { success: false, message: 'Provider "openai_compatible" 未配置' }
    if (!config.apiKey) return { success: false, message: 'API Key 未配置' }
    if (!config.enabled) return { success: false, message: 'Provider 已禁用' }
    if (!config.baseUrl) return { success: false, message: 'Base URL 未配置' }
