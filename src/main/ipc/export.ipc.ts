  ...
  ipcMain.handle(IPC_CHANNELS.EXPORT_ALL_DATA_JSON, async () => {
    try {
      ...
      const data = {
        ...
        memory_suggestions: db.prepare('SELECT * FROM memory_suggestions').all(),
        project_memory_items: db.prepare('SELECT * FROM project_memory_items').all(),
        user_interventions: db.prepare('SELECT * FROM user_interventions').all(),
